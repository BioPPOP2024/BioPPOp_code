...
% Type 0: [f,t,cl,sc] =  sp2a_m1(0,sp1,dat2,samp_rate,seg_pwr,opt_str)
% Type 1: [f,t,cl,sc] =  sp2a_m1(1,sp1,dat2,trig_times,duration,samp_rate,seg_pwr,opt_str)
% Type 2: [f,t,cl,sc] =  sp2a_m1(2,sp1,dat2,trig_times,offset,seg_pts,samp_rate,seg_pwr,opt_str)
% Function to calculate spectra, coherence, phase & cumulant for 1 spike train and 1 time series.
...
%  n  Normalise time series dat2 to unit variance within each segment.
...
%
% Options examples:
%  to set all options on (5% split taper), set opt_str='c h i m n r s t w1'
...
pts_tot=length(dat2);           % Determine size of data vector.
% Check if spike times exceed length of time series array.
if (max(sp1)>pts_tot)    
  error(' Spike times exceed time series length');
end
...
while (any(options))              % Parse individual options from string.
  ...
  switch (opt(1))
    ...
    case 'm'             % Mains/line frequency suppression option.
      flags.line=1;
    case 'n'             % Normalisation of time series to unit variance within each segment.
      norm_chan=1;
...
for ind_off=1:length(offset)            % Loop across all offset values.
  for ind=1:seg_tot                        % Loop across columns/segments.
    ...
    if rect_chan_1
      rd2(1:seg_pts,ind)=abs(rd2(1:seg_pts,ind));            % Rectification of time series (Full wave).  
    end
    if trend_chan_1                               % Linear trend removal.
      p=polyfit(trend_x(1:seg_pts,1),rd2(1:seg_pts,ind),1);                 % Fit 1st order polynomial.
      rd2(1:seg_pts,ind)=rd2(1:seg_pts,ind)-p(1)*trend_x(1:seg_pts,1)-p(2); % Subtract from time series.
    end
    if norm_chan
...
for ind_off=1:length(offset)
  ...
  if (flags.inv)      % Input and output channels inverted with 'i' option.
    ...
    cl(ind_off).N1=0;                              % N1, No of events in ch 2. (zero for TS data)
    cl(ind_off).P2=N1/(cl(ind_off).samp_tot*2*pi); % P2, mean intensity of events from sp1.
    cl(ind_off).P1=0;                              % P1, mean intensity ch2. (zero for TS data)
  else
    cl(ind_off).N1=N1;                             % N1, No of events analysed from sp1.
    cl(ind_off).N2=0;                              % N2, No of events in ch 2. (zero for TS data)
    cl(ind_off).P1=N1/(cl(ind_off).samp_tot*2*pi); % P1, mean intensity of events from sp1.
    cl(ind_off).P2=0;                              % P2, mean intensity ch2. (zero for TS data)
  end
