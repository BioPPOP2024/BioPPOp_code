function psptf_ph1(f,cl,freq,t_inc,n_contour,colorbar_flag,line_flag,limits_flag)
% function psptf_ph1(f,cl,freq,t_inc,n_contour,colorbar_flag,line_flag,limits_flag)
% Function to display time dependent input auto spectral estimate in current subplot window.
%
...
%    limits_flag    Optional flag to control limits - 0:[-pi, pi] (default), 1: Auto scale.
%
% function psptf_ph1(f,cl,freq,t_inc,n_contour,colorbar_flag,line_flag,limits_flag)
...
% Contour spacing
ph_int=2*pi/(n_contour+1);
ph_contour=[-pi, (-pi+ph_int:ph_int:pi-ph_int), pi];

% Plot Time dependent Phase.
[C,h]=contourf(squeeze(f(1:freq_pts,5,1:n_seg)),ph_contour);
if (limits_flag)
  set(gca,'CLim',[min(min(f(1:freq_pts,5,1:n_seg))) max(max(f(1:freq_pts,5,1:n_seg)))]);   
else
  set(gca,'CLim',[-pi pi]);   
end
if (line_flag==0)
