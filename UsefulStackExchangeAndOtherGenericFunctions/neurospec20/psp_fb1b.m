function psp_fb1b(f,cl,freq,label)
...
%   f_cl=[f_max*0.96,max(f(1:freq_pts,3));
%         f_max*0.96,max(f(1:freq_pts,3))-2*cl.f_c95];
%   plot(f(1:freq_pts,1),f(1:freq_pts,3),'k-', f_cl(:,1), f_cl(:,2),'k-', 'LineWidth', 2);
%   axis([0,freq,-Inf,Inf]);
  
    uppercl = f(1:freq_pts,3) + cl.f_c95;
  lowercl = f(1:freq_pts,3) - cl.f_c95;
  fill([f(1:freq_pts,1); flipud(f(1:freq_pts,1))], [uppercl; flipud(lowercl)], 'r', 'EdgeColor', 'none', 'FaceAlpha', 0.2)
  hold on
  plot(f(1:freq_pts,1),f(1:freq_pts,3),'r-');   
    %hold off
  axis([0,freq,-Inf,Inf]);
  set(gca,'xticklabel',[]);
end  
